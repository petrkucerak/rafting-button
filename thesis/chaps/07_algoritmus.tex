...
První dvě zmiňované nejistoty, konkrétně {\sbf mimoběžnost hodin} ({\em clock skew}) a {\sbf drift hodin}, se řeší synchronizací času. Podle zdroje referenčního času rozlišujeme synchronizaci na interní a externí. {\sbf Interní} se snaží o minimalizaci časového rozdílu $\delta$ mezi dvěma zařízeními ($d_i$, $d_j$), tedy
...
Pro implementaci poměrně jednoduchý a v DS, kde se čas synchronizuje napřímo mezi zařízeními, je z pohledu latence vysoce efektivní {\sbf Cristianův algoritmus}. Algoritmus předpokládá, že je doba přenosu symetrická. Přesně funguje tak, že pokud zařízení $A$ chce synchronizovat čas od zařízení $B$,
\begitems
...
Kromě synchronizace času můžeme v DS řešit i {\sbf kauzalitu}. Tedy aby bylo zachování pořadí událostí. K této problematice může vyžít fyzických hodin se synchronizovaným časem nebo logických hodin. {\sbf Logické hodiny} jsou takové, které se inkrementují v případě nějaké události jako je přijmutí či odeslání zprávy. Jednotlivé události pak jsou mezi sebou ve vztahu {\em stalo se před}.
...
\begitems
* Všechny zařízení, která nehavarovala, se musejí {\sbf shodnout na výstupu}.
...
Na začátku volby lídra jsou všechna zařízení {\em následovníci}. Ti očekávají zprávu od {\em lídra} nebo od {\em kandidáta} na lídra. Lídři v průběhu své vlády posílají {\em heartbeats}, aby si udrželi autoritu. Jakmile {\em následník} neobdrží zprávu do určitého timeoutu, předpokládá že {\em lídr} havaroval a iniciuje volbu nového lídra. Zařízení, které spustí volbu nového lídra: zvýší číslo epochy, změní svůj stav na {\em kandidát}, zahlasuje pro sebe, pošle $"RequestVote"$ všem ostatním serverům a čeká na jeden z následujících stavů:
\begitems
* obdrží hlasy od většiny serverů -> změní se na {\em lídr} a pošle {\em heartbeat} ostatní procesům,
...
Proto platí první podmínka říkající, že jelikož dosavadní logika fungování Raftu bezpečnostní invariant negarantuje, snaží se Raft zvolit lídra, který má nejúplnější log. Tj. epochu nejvyšší epochou případně pokud mají stejnou epochu, tak s nejvyšším indexem. A druhá, která říká, že aby lídr požadoval záznam za potvrzený musí být:
...
K {\sbf opravě logu následovníků} dochází tak, že nový {\em lídr} musí udělat logy následovníků konzistentních se svým logem. Tj. smazat přebytečné záznamy a doplnit chybějící záznamy. V praxi se oprava implementuje tak, že {\em lídr} určuje proměnnou $"nextIndex"$ pro každého {\em následovníka} index další záznamu logu, který by měl být odeslán {\em následovníkovi}, inicializován na $1$ + index poslední záznam {\em lídra}. Pokud kontrola konzistence $"AppendEntries"$ selže, sníží $"nextIndex"$ o jednu a zkusí znovu. {\em V podstatě prvně zjistí, jak moc do historie se musí vrátit. Tam vše ustřihne a naplní zásobník novými logy.}
...
Může se stát, že {\em lídr} havaruje poté, co už ale vykonal příkaz před odesláním odpovědi. Existuje tedy riziko opakovaného vykonání příkazu. Řešením je, že každý příkaz má jedinečné ID příkazu. Při přijmutí příkazu, pak vždy {\em lídr} zkontroluje zdali již ID není na zařízení uloženo.
